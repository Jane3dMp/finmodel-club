...
switch ($action) {
    ...
    //     короткий запрос на ребёнка. Жанна: «мне только 1 филиал» — по остальным НЕ ходим,
    //     иначе на каждого ненайденного уходит обход всей CRM (это и роняло запрос).
    //     removed=1 (архив) отдаём отдельным признаком: клиент сам решает, показывать ли.
    case 'customersByIds':
        @set_time_limit(90);
        ...
        // филиал: явно переданный → клубный из config (realization_branch_names) → дефолтный
        $bid0 = (int)($in['branch'] ?? 0) ?: ((alfa_realization_branches()[0] ?? 0) ?: alfa_branch());
        $host = 'https://' . alfa_host(); $token = alfa_token();
        $out = []; $missing = [];
        foreach ($ids as $cid) {
            $c = null;
            // ⚠️ archive=1 обязательно: без него Alfa не отдаёт клиентов из архива, и ребёнок,
            //    оставшийся в группе после архивации, выглядел бы «нет в справочнике».
            foreach ([['id' => $cid, 'page' => 0, 'count' => 1],
                      ['id' => $cid, 'page' => 0, 'count' => 1, 'removed' => 1]] as $body) {
                $r = alfa_http('POST', "$host/v2api/$bid0/customer/index", $body, $token, true, 8);
                foreach ((is_array($r['items'] ?? null) ? $r['items'] : []) as $it) {
                    if ((int)($it['id'] ?? 0) === $cid) { $c = $it; break 2; }
                }
            }
    ...
    // --- АБОНЕМЕНТЫ ДЕТЕЙ: купили или только сходили на пробное ---
    //     Пачками и отдельно от занятий: это запрос на каждого ребёнка, и внутри загрузки
    //     занятий он удвоил бы её стоимость.
    case 'kidsTariffs':
        @set_time_limit(180);
        $branches = alfa_realization_branches();
        $ids = array_slice((array)($in['ids'] ?? []), 0, 15);
        json_out(['ok' => true] + alfa_kids_tariffs($ids, $branches, !empty($in['force'])));
        break;
    ...
    case 'kidsHistory':
        @set_time_limit(180);
        $branches = alfa_realization_branches();
        $ids = array_slice((array)($in['ids'] ?? []), 0, 25);   // пачка, дальше клиент шлёт ещё
        $d = alfa_iso((string)($in['date'] ?? date('Y-m-d')));
        json_out(['ok' => true, 'date' => $d] + alfa_kids_history($ids, $d, $branches));
        break;

    // --- ПРОБНЫЕ ЗА ДЕНЬ: утром «кого ждём», вечером «кто дошёл» ---
    //     Пробный опознаётся по СУММЕ списания: 0 значит, что шаблон абонемента ещё не
    //     проставили (его часто ставят прямо на занятии), и такой ребёнок почти наверняка
    //     пробник. Доступно администраторам, а не только владельцу: это ежедневная работа админа.
    case 'trialsDay':
        @set_time_limit(180);
        $branches = alfa_realization_branches();
        if (!$branches) json_out(['ok' => false, 'error' => 'Филиал «Пожарный» не найден в Alfa'], 400);
