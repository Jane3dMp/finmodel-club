...
@set_time_limit(0);
/* Окно пересчёта. По будням семь дней — этого хватает на обычные исправления посещаемости и
   не растягивает прогон. По ВОСКРЕСЕНЬЯМ заглядываем на месяц назад: правка старше недели
   иначе не всплыла бы никогда, а раз в неделю такой проход клуб переживёт. */
$defBack = (date('N') === '7') ? 30 : 7;
$days  = (int)($_GET['days']  ?? ($argv[1] ?? $defBack));   // сколько дней НАЗАД пересчитать (факт)
$ahead = (int)($_GET['ahead'] ?? ($argv[2] ?? 14));   // минимум дней ВПЕРЁД (прогноз по расписанию)
$days  = max(1, min(60, $days));
$ahead = max(0, min(90, $ahead));
/* ⚠️ Воскресный проход на месяц назад применяем ПОСЛЕ разбора аргументов, а не как умолчание.
   В боевом crontab задание записано как «cron_realization.php 7 14» — явная семёрка перебивала
   умолчание, и месячная проверка правок задним числом не запускалась ни разу. Полагаться на то,
   что в расписании когда-нибудь уберут аргумент, нельзя: проверка молча не работала бы дальше.
   Пересчёт идемпотентен, поэтому расширить окно безопасно — просто дольше. */
if (date('N') === '7') $days = max($days, 30);
