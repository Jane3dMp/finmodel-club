...
// 2 и 3 ребёнка — это недобранная группа, самая частая причина убытка; она должна быть видна
check('в таблице есть колонки 2 и 3 ребёнка',
      h.indexOf('<th class="num">2</th>') > 0 && h.indexOf('<th class="num">3</th>') > 0);
eq('при 2 детях ставка педагога как при 4 — ниже таблица ставок не идёт',
   API._fmlLesson('Скретч', 2, 0).wage, API._fmlLesson('Скретч', 4, 0).wage);
check('и при этом занятие в убытке', API._fmlLesson('Скретч', 2, 60).profit < 0,
      String(API._fmlLesson('Скретч', 2, 60).profit));
