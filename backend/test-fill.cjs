...
check('и объяснено, почему цифры разные',
      h.indexOf('планировщик считает, сколько мест должно открываться каждую неделю') > 0,
      'нет пояснения про расхождение');
...
/* ================= ЭТАЛОН НЕ ДОЛЖЕН СРАВНИВАТЬ ПЕРИОД САМ С СОБОЙ =================
   Владелец выбрала месяцем-образцом сентябрь 2026, а периодом — неделю 31.08–06.09 внутри
   этого же сентября. Тогда одни и те же дни стоят и в эталоне, и в «факте сейчас», а разница
   берётся из одной только длины отрезка и не значит ничего. */
{
  const keepBase = ctx.S.fillBase;

  // сохранённый образец пересекается с периодом — подставляем умолчание и говорим об этом
  ctx.S.fillBase = '2026-09';
  ctx._fillPeriod = 'w:2026-08-31';
  ...
  const both = API._fillHtml();
  check('когда подменить нечем — предупреждение', both.indexOf('Месяц-образец пересекается') > 0,
        both.slice(Math.max(0, both.indexOf('Эталон') - 20), both.indexOf('Эталон') + 300));
  check('и объяснено, почему это бессмысленно', both.indexOf('сравнивает период сам с собой') > 0);
  ctx._fillPeriod = 'w:2026-08-31';
  ...
  ctx.S.fillBase = '2026-03';
  const good = API._fillHtml();
  check('у прошлого месяца предупреждения нет', good.indexOf('Месяц-образец пересекается') < 0);
