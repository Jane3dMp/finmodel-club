...
const ctx = {
  S: { fillPlan: { '20': 6 }, plan: [{ name: 'Английский', perGroup: 6, groups: 3, visits: 2 }],
       assume: { weeksPerMonth: 4 }, salesCfg: null },
  ...
  _fillRange: { from: '', to: '' },        // «свой период»: пункт есть всегда, даты пустые
...
/* ================= 5. свод за месяц ================= */
/* Март оставлен как окно длиннее недели: на нём видно, что месячный знаменатель кратно
   больше недельного. Эталоном март больше НЕ является — эталон это тот же период год
   назад, см. секцию ниже. */
...
check('план финмодели показан', h.indexOf('Детоместа: план ↔ открылось ↔ заполнено') > 0);
// ⚠️ три цифры, две из которых называются «детоместа»: план недели, фактически открывшиеся
// места и списания. Без разведения они читаются как ошибка — Жанна на это и наткнулась.
check('план и «открылось» разведены', h.indexOf('Открылось: вместимость ×') > 0);
/* ⚠️ В этой фикстуре окно посчитано не целиком (3 дня из 7), поэтому вместо подсказки про
   каникулы обязана стоять честная оговорка: раньше на её месте было «разница — это места
   занятий, которых не было: каникулы, праздники», хотя занятия не отменяли — дни просто
   не посчитаны. Подсказку про каникулы проверяем ниже, на полном окне. */
check('про недосчитанное окно сказано прямо', h.indexOf('Окно посчитано не целиком') > 0,
      h.slice(Math.max(0, h.indexOf('Детоместа: план')), h.indexOf('Детоместа: план') + 900));
check('и названо, сколько дней из скольких', h.indexOf('3 дня из 7') > 0);
check('план приведён к посчитанным дням', h.indexOf('на посчитанные 3 дня из 7') > 0);
check('и выполнение подписано', h.indexOf('Выполнение плана (за посчитанные дни)') > 0);
check('эталон показан', h.indexOf('Эталон загрузки') > 0);
// ⚠️ прошлого года в этом хранилище нет вовсе — блок обязан сказать «нет данных», а не «0%»:
// нуль на месте эталона читается как обвал загрузки и пугает зря.
check('эталон честно говорит, что прошлого года нет', h.indexOf('в хранилище ничего нет') > 0,
...
/* ================= ЭТАЛОН: ТОТ ЖЕ ПЕРИОД ГОД НАЗАД =================
   Правило Жанны 08.09.2026: «если я ставлю 2–7, то и в прошлом сентябре это должно быть
   первая неделя с 3 по 8 — среда–понедельник». Значит сдвиг ровно на 364 дня (52 недели), а
   НЕ «минус год»: только так сохраняется день недели. Для клуба это не придирка — суббота
   даёт втрое больше будня, и календарный сдвиг сам по себе нарисовал бы падение.
   Прежний месяц-образец (S.fillBase) убран: он сравнивал неделю с месяцем, требовал досчитать
   целый месяц ради одной недели и умел незаметно пересечься с самим периодом. */
{
  /* --- арифметика сдвига --- */
  eq('среда → среда', API._fillShift('2026-09-02'), '2025-09-03');
  eq('понедельник → понедельник', API._fillShift('2026-08-31'), '2025-09-01');
  eq('воскресенье → воскресенье', API._fillShift('2026-09-06'), '2025-09-07');
  // ровно тот ответ, который Жанна назвала руками: 2–7 сентября 2026 → 3–8 сентября 2025
  eq('её пример, начало', API._fillShift('2026-09-02'), '2025-09-03');
  eq('её пример, конец', API._fillShift('2026-09-07'), '2025-09-08');
  check('день недели сохранён на начале окна',
        API._fillWd('2026-09-02') === API._fillWd(API._fillShift('2026-09-02')),
        API._fillWd('2026-09-02') + ' ≠ ' + API._fillWd(API._fillShift('2026-09-02')));
  check('и на конце окна',
        API._fillWd('2026-09-07') === API._fillWd(API._fillShift('2026-09-07')));
  eq('пара дней недели', API._fillWdPair('2026-09-02', '2026-09-07'), 'ср–пн');
  eq('один день — без пары', API._fillWdPair('2026-09-02', '2026-09-02'), 'ср');

  // ⚠️ окно обязано быть ТОЙ ЖЕ длины — иначе проценты сравнивать нельзя
  const len = (a, b) => Math.round((new Date(b + 'T12:00:00') - new Date(a + 'T12:00:00')) / 86400000);
  eq('длина окна не меняется',
     len(API._fillShift('2026-08-31'), API._fillShift('2026-09-06')), len('2026-08-31', '2026-09-06'));
  // ⚠️ и оно НИКОГДА не пересекается с самим периодом — этим грешил месяц-образец
  check('прошлогоднее окно кончается раньше периода', API._fillShift('2026-09-06') < '2026-08-31');
  const weeks = API._fillPeriods().filter(p => p.kind === 'w');
  check('и так у каждой недели в списке', weeks.every(p => API._fillShift(p.to) < p.from),
        'пересеклось: ' + (weeks.find(p => API._fillShift(p.to) >= p.from) || {}).k);

  // подпись: год обязателен, иначе прошлогодняя неделя неотличима от нынешней
  eq('подпись окна', API._fillRangeLabel('2025-09-01', '2025-09-07'), '01.09–07.09.2025');
  eq('один день — полная дата', API._fillRangeLabel('2025-09-01', '2025-09-01'), '01.09.2025');
  eq('без дат — пусто', API._fillRangeLabel('', ''), '');
}

/* --- блок эталона на данных: год назад было 100%, сейчас 50% --- */
{
  const keepFill = ctx._fillStore.fill, keepPlan = ctx.S.plan, keepFP = ctx.S.fillPlan;
  ctx.S.plan = [{ name: 'Английский', perGroup: 6, groups: 3, visits: 2 }];
  ctx.S.fillPlan = {};                       // вместимость 6 придёт из планировщика
  /* Неделя 31.08–06.09.2026 (пн–вс) и её прошлогодний двойник 01–07.09.2025 (пн–вс).
     Год назад группа была полной, сейчас — половина: блок обязан сказать «хуже» словами,
     а не просто показать два процента рядом. */
  ctx._fillStore.fill = {
    '2026-09-01': { '10': [1, 6, 3, 0, 3, 90] },        // 3 из 6 → 50%
    '2025-09-01': { '10': [1, 6, 6, 0, 6, 180] },       // все семь дней по 6 из 6 → 100%
    '2025-09-02': { '10': [1, 6, 6, 0, 6, 180] },
    '2025-09-03': { '10': [1, 6, 6, 0, 6, 180] },
    '2025-09-04': { '10': [1, 6, 6, 0, 6, 180] },
    '2025-09-05': { '10': [1, 6, 6, 0, 6, 180] },
    '2025-09-06': { '10': [1, 6, 6, 0, 6, 180] },
    '2025-09-07': { '10': [1, 6, 6, 0, 6, 180] },
  };
  ctx._fillPeriod = 'w:2026-08-31';

  const B = API._fillTotals(API._fillRows(API._fillAgg('2025-09-01', '2025-09-07')));
  eq('год назад: мест было', B.seats, 42);              // 6 × 7 занятий
  eq('год назад: списаний', B.fact, 42);
  eq('год назад: загрузка', Math.round(B.pct), 100);

  const h = API._fillHtml();
  check('заголовок говорит, с чем сравниваем',
        h.indexOf('Эталон загрузки — тот же период год назад') > 0, h.slice(0, 200));
  check('прошлогоднее окно названо датами', h.indexOf('01.09–07.09.2025') > 0);
  check('и днями недели', h.indexOf('(пн–вс)') > 0);
  check('строка «Год назад»', h.indexOf('Год назад — 01.09–07.09.2025') > 0);
  check('строка «Сейчас»', h.indexOf('Сейчас — ') > 0);
  check('разница в процентных пунктах', h.indexOf('п.п.') > 0);
  check('и она посчитана как 100 − 50', h.indexOf('50 п.п.') > 0,
        (h.match(/[+−]\d+ п\.п\./g) || []).join(','));
  // ⚠️ одни проценты не отвечают на вопрос «и что?» — под цифрами должен быть вывод словами
  const txt = h.replace(/<[^>]+>/g, '');
  check('вывод словами: хуже', txt.indexOf('Загружаем хуже, чем год назад') > 0,
        (txt.match(/Загружаем[^.]*\./) || [''])[0]);
  check('и сказано, на сколько мест из ста',
        txt.indexOf('пустует на 50 из каждых 100 мест больше') > 0);
  check('оговорено, почему проценты, а не списания',
        h.indexOf('Сравниваем проценты, а не списания') > 0);
  // ⚠️ вместимость берётся сегодняшняя — про перекос надо сказать вслух, а не молчать
  check('сказано про сегодняшнюю вместимость',
        h.indexOf('Вместимость (детоместа) везде берётся сегодняшняя') > 0);

  // ⚠️ месяца-образца больше нет: ни селектора, ни разговоров про пересечение
  check('селектор месяца-образца убран', h.indexOf('fillBaseSet') < 0);
  check('и предупреждения про пересечение тоже', h.indexOf('пересекается с выбранным периодом') < 0);
  check('окно полное — про недосчёт молчим', h.indexOf('посчитан не весь') < 0);

  /* Обратный случай: сейчас лучше, чем год назад. Знак должен перевернуться вместе со словом,
     иначе «+» с «хуже» разойдутся и раздел начнёт врать. */
  ctx._fillStore.fill = {
    '2026-09-01': { '10': [1, 6, 6, 0, 6, 180] },       // 100%
    '2025-09-01': { '10': [1, 6, 3, 0, 3, 90] },        // 50%
  };
  const up = API._fillHtml(), upTxt = up.replace(/<[^>]+>/g, '');
  check('вывод словами: лучше', upTxt.indexOf('Загружаем лучше, чем год назад') > 0,
        (upTxt.match(/Загружаем[^.]*\./) || [''])[0]);
  check('и знак плюс', up.indexOf('+50 п.п.') > 0, (up.match(/[+−]\d+ п\.п\./g) || []).join(','));

  /* Разница меньше 3 п.п. — это шум, а не новость: слово должно быть «примерно так же». */
  ctx._fillStore.fill = {
    '2026-09-01': { '10': [1, 6, 6, 0, 6, 180] },
    '2025-09-01': { '10': [1, 6, 6, 0, 6, 180] },
  };
  const same = API._fillHtml().replace(/<[^>]+>/g, '');
  check('одинаково — так и сказано', same.indexOf('Загружаем примерно так же, как год назад') > 0,
        (same.match(/Загружаем[^.]*\./) || [''])[0]);

  ctx._fillStore.fill = keepFill; ctx.S.plan = keepPlan; ctx.S.fillPlan = keepFP;
  ctx._fillPeriod = 'w:2026-08-31';
}
...
/* ================= ПРОШЛОГОДНЕЕ ОКНО ПОСЧИТАНО НЕ ВЕСЬ =================
   Живой случай 08.09.2026: эталон стоял на неполном окне, и 74% по трём дням выглядели ровно
   так же, как 74% по семи. Отличить их можно ТОЛЬКО по строке-предупреждению, поэтому блок
   обязан сказать это вслух и дать досчитать, не уходя из выбранного периода.
   Прошлый год в хранилище не копится сам: ночной пересчёт ходит по свежим дням, а год назад
   подтягивается только этой кнопкой. Значит пустое окно — норма, а не поломка. */
{
  const keepFill = ctx._fillStore.fill, keepPlan = ctx.S.plan, keepFP = ctx.S.fillPlan;
  ctx.S.plan = [{ name: 'Английский', perGroup: 6, groups: 3, visits: 2 }];
  ctx.S.fillPlan = {};
  ...
  /* --- дыры в окне: 4 из 7 прошлогодних дней не посчитаны --- */
  ctx._fillStore.fill = {
    '2026-09-01': { '10': [1, 6, 3, 0, 3, 90] },
    '2025-09-01': { '10': [1, 6, 6, 0, 6, 180] },
    '2025-09-02': { '10': [1, 6, 6, 0, 6, 180] },
    '2025-09-03': { '10': [1, 6, 6, 0, 6, 180] },
  };
  const miss = API._fillRangeMissing('2025-09-01', '2025-09-07');
  eq('не хватает 4 дней', miss.length, 4);
  check('и это именно дни окна', miss.every(d => d >= '2025-09-04' && d <= '2025-09-07'), miss.join(','));
  check('уже посчитанные не просим заново', miss.indexOf('2025-09-01') < 0);

  const h = API._fillHtml();
  check('сказано, что окно неполное', h.indexOf('Прошлый год посчитан не весь') > 0,
        h.slice(Math.max(0, h.indexOf('Эталон загрузки')), h.indexOf('Эталон загрузки') + 900));
  check('названо, сколько не хватает', h.indexOf('не хватает 4 дней') > 0);
  check('и есть кнопка досчитать', h.indexOf("fillPullRange('2025-09-01','2025-09-07')") > 0);
  check('оговорено, что процент по неполному окну',
        h.indexOf('Процент считается по тем дням, что есть') > 0);
  // ⚠️ проценты всё равно показываем: по трём дням они лучше, чем ничего, — но с оговоркой
  check('процент при этом посчитан', h.indexOf('Год назад — ') > 0);

  /* --- прошлого года нет вовсе: не «0%», а честное «ничего нет» + кнопка --- */
  ctx._fillStore.fill = { '2026-09-01': { '10': [1, 6, 3, 0, 3, 90] } };
  const empty = API._fillHtml();
  check('пустое окно названо пустым', empty.indexOf('в хранилище ничего нет') > 0,
        empty.slice(Math.max(0, empty.indexOf('Эталон загрузки')), empty.indexOf('Эталон загрузки') + 600));
  check('и предложено посчитать все 7 дней', empty.indexOf('посчитать 7 дней из Alfa') > 0);
  // ⚠️ ноль процентов вместо «нет данных» — самое опасное: выглядит как провал загрузки
  check('нуля процентов не рисуем', empty.indexOf('Год назад — ') < 0);
  check('и разницу не выдумываем', empty.indexOf('п.п.') < 0);

  /* --- окно посчитано целиком — предупреждения быть не должно --- */
  const full = { '2026-09-01': { '10': [1, 6, 3, 0, 3, 90] } };
  for (let d = 1; d <= 7; d++) full['2025-09-0' + d] = { '10': [1, 6, 6, 0, 6, 180] };
  ctx._fillStore.fill = full;
  eq('полное окно ничего не просит', API._fillRangeMissing('2025-09-01', '2025-09-07').length, 0);
  check('и предупреждения нет', API._fillHtml().indexOf('посчитан не весь') < 0);

  /* --- будущие дни не просим: период, который ещё идёт, «неполон» законно --- */
  eq('будущий месяц не просим считать', API._fillRangeMissing('2026-12-01', '2026-12-31').length, 0);
  eq('до сегодня включительно — просим', API._fillRangeMissing('2026-09-05', '2026-09-09').length, 3);
  eq('без дат — пустой список', API._fillRangeMissing('', '').length, 0);
...
/* ================= ЭТАЛОН: ДВА ОКНА — ОДНО ПРАВИЛО ЧИСЛИТЕЛЯ =================
   Дефект, найденный разбором 09.09.2026. Галочки «что считать занятым местом» применимы только
   к окну, у которого есть разложение по деньгам (attPaid/attFree/attZero). У прошлогоднего окна
   его обычно нет: дни подтянуты кнопкой ДО появления этих полей, и _fillRows молча падает на
   списания. Пока смотрят одно окно, это безобидно. Но эталон сравнивает ДВА, и тогда «88% по
   галочкам» сравнивались с «63% по списаниям»: на ПОЛНОСТЬЮ одинаковых данных двух недель экран
   объявлял «+25 п.п., загружаем лучше». Предупреждения не было ни одного — дыр в окне нет,
   дни есть, они лишь короткие, и _fillRangeMissing даёт ноль. */
{
  const keepFill = ctx._fillStore.fill, keepPlan = ctx.S.plan, keepFP = ctx.S.fillPlan,
        keepCats = ctx.S.fillCats;
  ctx.S.plan = []; ctx.S.fillPlan = { '10': 8 };
  ctx.S.fillCats = { full: 1, trial: 0, zero: 1, none: 1, miss: 1 };   // Жанна отметила «за 0»
  ctx._fillPeriod = 'w:2026-08-31';

  /* Одна группа, вместимость 8, по занятию в день. Обе недели заданы ОДИНАКОВО:
     за 7 дней 56 детомест, 35 списаний, пришли 49 = 35 по полной цене + 14 «за 0».
     Разница только в формате хранения. */
  const NEW = [1, 8, 5, 0, 7, 150, 5, 2, 0, 0, 2, {}, {}];   // есть attZero
  const OLD = [1, 8, 5, 0, 7, 150];                           // старый формат, шесть полей
  const mk = (nowRow, lyRow) => {
    const F = {};
    for (let d = 1; d <= 7; d++) F['2025-09-0' + d] = { '10': lyRow.slice() };
    F['2026-08-31'] = { '10': nowRow.slice() };
    for (let d = 1; d <= 6; d++) F['2026-09-0' + d] = { '10': nowRow.slice() };
    return F;
  };

  /* --- перекос: сейчас новый формат, год назад старый --- */
  ctx._fillStore.fill = mk(NEW, OLD);
  const Tn = API._fillTotals(API._fillRows(API._fillAgg('2026-08-31', '2026-09-06')));
  const Bn = API._fillTotals(API._fillRows(API._fillAgg('2025-09-01', '2025-09-07')));
  eq('детоместа у окон совпадают', Tn.seats, Bn.seats);
  check('текущее окно умеет галочки', Tn.split === true);
  check('прошлогоднее — нет', Bn.split === false);
  eq('и его числитель падает на списания', Bn.cnt, Bn.fact);
  // ⚠️ вот из-за чего бралась разница: pct у окон посчитан разными правилами
  check('сырые проценты действительно разошлись', Math.round(Tn.pct) !== Math.round(Bn.pct),
        Tn.pct + ' vs ' + Bn.pct);
  // а по списаниям — сходятся, потому что данные одинаковы
  eq('по списаниям окна равны', Math.round(Tn.pctFact), Math.round(Bn.pctFact));
  eq('дыр в прошлогоднем окне нет', API._fillRangeMissing('2025-09-01', '2025-09-07').length, 0);

  const h = API._fillHtml();
  const txt = h.replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ');
  check('разница не выдумана', txt.indexOf('Разница +0 п.п.') > 0,
        (txt.match(/Разница [+−]\d+ п\.п\./) || [''])[0]);
  check('и сказано словами, что одинаково', txt.indexOf('Загружаем примерно так же, как год назад') > 0);
  // ⚠️ молчать нельзя: правило сравнения сменилось, и об этом надо сказать
  check('оговорка про списания есть', h.indexOf('Сравниваем по списаниям, а не по галочкам') > 0);
  check('названо, у какого окна нет разбивки', txt.indexOf('нет у прошлогоднего окна') > 0, txt.slice(0, 0) || txt.match(/нет у [а-я ]+/) || '');
  check('и дана кнопка пересчитать прошлый год',
        h.indexOf("fillPullRange('2025-09-01','2025-09-07',true)") > 0);
  check('лишней кнопки на текущий период нет', h.indexOf("fillPullRange('2026-08-31','2026-09-06',true)") < 0);
  // ⚠️ подпись под процентом обязана давать этот же процент
  check('под «Год назад» стоит числитель процента', txt.indexOf('63% 35 из 56 детомест') > 0,
        (txt.match(/Год назад[^%]*%\s*\d+ из \d+/) || [''])[0]);
  check('и под «Сейчас» тот же', txt.indexOf('Сейчас — неделя 31.08–06.09 7 занятий за 7 дней 63% 35 из 56') > 0,
        (txt.match(/Сейчас[^%]*%\s*\d+ из \d+/) || [''])[0]);

  /* --- перекос наоборот: год назад пересчитан, а текущий период нет --- */
  ctx._fillStore.fill = mk(OLD, NEW);
  const h2 = API._fillHtml(), t2 = h2.replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ');
  check('перекос в другую сторону тоже сведён', t2.indexOf('Разница +0 п.п.') > 0,
        (t2.match(/Разница [+−]\d+ п\.п\./) || [''])[0]);
  check('и палец указан на выбранный период', t2.indexOf('нет у выбранного периода') > 0);
  check('кнопка — на выбранный период', h2.indexOf("fillPullRange('2026-08-31','2026-09-06',true)") > 0);

  /* --- разбивки нет ни у кого: сравнение честное, но кнопки нужны обе --- */
  ctx._fillStore.fill = mk(OLD, OLD);
  const h3 = API._fillHtml(), t3 = h3.replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ');
  check('оба окна без разбивки — так и сказано', t3.indexOf('нет у обоих окон') > 0);
  check('кнопка на прошлый год', h3.indexOf("fillPullRange('2025-09-01','2025-09-07',true)") > 0);
  check('и кнопка на выбранный период', h3.indexOf("fillPullRange('2026-08-31','2026-09-06',true)") > 0);
  check('разница всё равно 0', t3.indexOf('Разница +0 п.п.') > 0);

  /* --- оба окна новые: галочки работают, оговорки нет --- */
  ctx._fillStore.fill = mk(NEW, NEW);
  const h4 = API._fillHtml(), t4 = h4.replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ');
  const T4 = API._fillTotals(API._fillRows(API._fillAgg('2026-08-31', '2026-09-06')));
  check('оба окна умеют галочки', T4.split === true);
  check('оговорки нет', h4.indexOf('Сравниваем по списаниям') < 0);
  check('разница 0', t4.indexOf('Разница +0 п.п.') > 0);
  // и теперь под процентом стоит числитель ПО ГАЛОЧКАМ, а не списания
  check('числитель по галочкам', t4.indexOf('88% 49 из 56 детомест') > 0,
        (t4.match(/Год назад[^%]*%\s*\d+ из \d+/) || [''])[0]);
  // ⚠️ галочки продолжают менять цифру — иначе мы бы починили дефект, сломав саму настройку
  ctx.S.fillCats = { full: 1, trial: 0, zero: 0, none: 0, miss: 1 };
  const t5 = API._fillHtml().replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ');
  check('снятая галочка «за 0» меняет проценты обоих окон', t5.indexOf('63% 35 из 56 детомест') > 0,
        (t5.match(/Год назад[^%]*%\s*\d+ из \d+/) || [''])[0]);
  check('но разница по-прежнему 0', t5.indexOf('Разница +0 п.п.') > 0);

  ctx._fillStore.fill = keepFill; ctx.S.plan = keepPlan; ctx.S.fillPlan = keepFP;
  ctx.S.fillCats = keepCats; ctx._fillPeriod = 'w:2026-08-31';
}

/* ================= ПОДПИСЬ ОКНА ЧЕРЕЗ НОВЫЙ ГОД =================
   Год брался только из даты конца, и прошлогоднее окно 30.12.2024–05.01.2025 подписывалось
   «30.12–05.01.2025» — читалось как окно ВНУТРИ 2025 года, вложенное в выбранную неделю
   29.12.2025–04.01.2026. Ровно то, от чего год в подписи и должен спасать. Та же подпись
   уходит в чат отделу продаж («свой период» в отчёте) и на кнопки досчёта. */
{
  eq('окно внутри года — год один раз',
     API._fillRangeLabel('2025-09-01', '2025-09-07'), '01.09–07.09.2025');
  eq('окно через Новый год — год у обеих дат',
     API._fillRangeLabel('2024-12-30', '2025-01-05'), '30.12.2024–05.01.2025');
  eq('и у выбранной недели тоже',
     API._fillRangeLabel('2025-12-29', '2026-01-04'), '29.12.2025–04.01.2026');
  eq('один день — полная дата', API._fillRangeLabel('2026-01-01', '2026-01-01'), '01.01.2026');
  eq('без дат — пусто', API._fillRangeLabel('', ''), '');
  // ⚠️ неделя через Новый год реально есть в списке периодов — проверяем на ней
  const wk = API._fillPeriods().find(p => p.k === 'w:2025-12-29');
  check('неделя через Новый год в списке есть', !!wk, 'нет w:2025-12-29');
  if (wk) {
    const lf = API._fillShift(wk.from), lt = API._fillShift(wk.to);
    eq('её прошлогоднее окно', lf + '..' + lt, '2024-12-30..2025-01-05');
    check('и подписано без вложения в текущую неделю',
          API._fillRangeLabel(lf, lt).indexOf('2024') > 0, API._fillRangeLabel(lf, lt));
  }
}


/* ================= ПЛАН И ФАКТ — ПО ОДНИМ ДНЯМ =================
   Дефект, найденный разбором 09.09.2026. План брался за ВСЁ окно, а факт — только за дни, которые
   есть в хранилище. У смены на 14 дней, где посчитано 8, «Выполнение плана» выходило 54% красным,
   а подсказка объясняла разницу «каникулами, праздниками, ещё не запущенными группами» — хотя
   занятия не отменяли: остальные дни ещё не наступили. */
{
  const keepFill = ctx._fillStore.fill, keepPlan = ctx.S.plan, keepFP = ctx.S.fillPlan,
        keepPeriod = ctx._fillPeriod, keepRange = ctx._fillRange, keepLib = ctx.courseFromLib;
  /* ⚠️ В библиотеке курсов у «Английского» свои визиты, и _fillModelPlan берёт их вместо
     вписанных в план. Здесь библиотеку убираем: проверяем арифметику плана, а не сверку с ней. */
  ctx.courseFromLib = () => null;
  ctx.S.plan = [{ name: 'Английский', perGroup: 10, groups: 1, visits: 7 }];   // 70 детомест/нед
  ctx.S.fillPlan = { '10': 10 };

  /* Свой период 01–14.09.2026, сегодня 07-е. Посчитано 7 дней (01–07), 07–14 впереди. */
  ctx._fillRange = { from: '2026-09-01', to: '2026-09-14' };
  ctx._fillPeriod = 'c';
  const F = {};
  for (let d = 1; d <= 7; d++) F['2026-09-0' + d] = { '10': [1, 10, 10, 0, 10, 300] };
  ctx._fillStore.fill = F;

  const agg = API._fillAgg('2026-09-01', '2026-09-14');
  eq('дней окна в хранилище', agg.days, 7);
  const Tt = API._fillTotals(API._fillRows(agg));
  eq('списаний за них', Tt.fact, 70);

  const h = API._fillHtml();
  /* План за всё окно: 70 × (14/7) = 140. Приведённый к 7 посчитанным дням: 70.
     Значит выполнение — 100%, а не 50%. */
  check('план за всё окно показан', h.indexOf('140') > 0, h.slice(h.indexOf('Детоместа: план'), h.indexOf('Детоместа: план') + 700));
  check('и приведённый к посчитанным дням тоже',
        h.indexOf('на посчитанные 7 дней из 14') > 0,
        h.slice(h.indexOf('Детоместа: план'), h.indexOf('Детоместа: план') + 900));
  // ⚠️ вот из-за чего было красное «54%»: факт за 7 дней делился на план за 14
  check('выполнение считается от приведённого плана', /Выполнение плана \(за посчитанные дни\)[\s\S]{0,200}?100%/.test(h),
        (h.match(/Выполнение плана[\s\S]{0,220}/) || [''])[0]);
  check('и половины из ниоткуда нет', h.indexOf('>50%<') < 0);
  // ⚠️ и объяснение не должно врать про отменённые занятия
  check('про каникулы молчим, пока окно недосчитано',
        h.indexOf('каникулы, праздники') < 0,
        (h.match(/каникулы[\s\S]{0,120}/) || [''])[0]);
  check('вместо этого честная причина', h.indexOf('Окно посчитано не целиком') > 0);

  /* Полное окно: оговорки нет, подсказка про каникулы возвращается, когда занятий и правда
     меньше плана. Берём 14 дней, но занятий только в семи — это уже настоящий недобор. */
  ctx._fillRange = { from: '2026-09-01', to: '2026-09-14' };
  const F2 = {};
  for (let d = 1; d <= 9; d++) F2['2026-09-0' + d] = { '10': [1, 10, 10, 0, 10, 300] };
  for (let d = 10; d <= 14; d++) F2['2026-09-' + d] = { '10': [0, 0, 0, 0, 0, 0] };   // день есть, занятий нет
  ctx._fillStore.fill = F2;
  const h2 = API._fillHtml();
  eq('всё окно в хранилище', API._fillAgg('2026-09-01', '2026-09-14').days, 14);
  check('оговорки про недосчёт нет', h2.indexOf('Окно посчитано не целиком') < 0);
  check('а подсказка про каникулы вернулась', h2.indexOf('каникулы, праздники') > 0,
        h2.slice(h2.indexOf('Детоместа: план'), h2.indexOf('Детоместа: план') + 900));

  ctx._fillStore.fill = keepFill; ctx.S.plan = keepPlan; ctx.S.fillPlan = keepFP;
  ctx._fillPeriod = keepPeriod; ctx._fillRange = keepRange; ctx.courseFromLib = keepLib;
}

/* ================= ТРИ ПОКОЛЕНИЯ ФОРМАТА: ЧТО МОЖНО УТВЕРЖДАТЬ =================
   Формат дневной строки детомест наращивали трижды, и у окна бывает любое поколение:
     1) шесть полей — только списания;
     2) + attPaid/attFree/noAttPaid — «кто был с деньгами и без них» (признак money);
     3) + attTrial/attZero — «пробное за 15» и «за 0» отдельно (признак split).
   ⚠️ Живой дефект (09.09.2026): остаток «пробных» считался всегда — att − paid − free. На первом
   поколении attPaid/attFree нулевые, поэтому остаток равен ВСЕМ пришедшим, и в чат отделу продаж
   уходило «Пришли 36: пробное 36» — 36 детей по полной цене объявлялись пробниками за 15 рублей.
   Это хуже, чем промолчать: цифра выглядит осмысленной, и оспорить её нечем.
   Но глушить остаток совсем нельзя: на ВТОРОМ поколении он верен. Отсюда два признака. */
{
  const keepFill = ctx._fillStore.fill, keepFP = ctx.S.fillPlan, keepCats = ctx.S.fillCats,
        keepPeriod = ctx._fillPeriod, keepRange = ctx._fillRange, keepPlan = ctx.S.plan;
  ctx.S.plan = []; ctx.S.fillPlan = { '10': 10 };
  ctx.S.fillCats = { full: 1, trial: 0, zero: 0, none: 0, miss: 1 };
  ctx._fillRange = { from: '2026-09-01', to: '2026-09-03' };
  ctx._fillPeriod = 'c';

  const put = (row) => {
    const F = {};
    for (let d = 1; d <= 3; d++) F['2026-09-0' + d] = { '10': row.slice() };
    ctx._fillStore.fill = F;
    return API._fillWho(API._fillRows(API._fillAgg('2026-09-01', '2026-09-03')));
  };

  /* --- первое поколение: корзин нет вовсе --- */
  const W1 = put([1, 10, 5, 0, 12, 150]);
  eq('пришли посчитаны', W1.att, 36);
  check('разбивки по деньгам нет', W1.money === false);
  eq('и пробных не выдумываем', W1.trial, 0);
  eq('и полной цены тоже', W1.paid, 0);
  const h1 = API._fillHtml();
  check('вместо плиток — причина', h1.indexOf('Разложения «кто был с деньгами и без них» за это окно нет') > 0,
        h1.slice(0, 400));
  check('пришедшие всё равно названы', h1.indexOf('<b>Пришли: 36.</b>') > 0);
  check('и дана кнопка пересчитать', h1.indexOf("fillPullRange('2026-09-01','2026-09-03',true)") > 0);
  check('плитки «полная цена» нет', h1.indexOf('пришли · полная цена') < 0);

  /* --- второе поколение: остаток = пробные, и это верно --- */
  // 12 пришли: 8 по полной цене, 1 без списания, значит 3 пробных остатком
  const W2 = put([1, 10, 5, 0, 12, 150, 8, 1, 2]);
  check('разбивка по деньгам есть', W2.money === true);
  check('а «за 0» отдельно ещё нет', W2.split === false);
  eq('полная цена', W2.paid, 24);
  eq('пробные остатком', W2.trial, 36 - 24 - 3);
  const h2 = API._fillHtml();
  check('плитки вернулись', h2.indexOf('пришли · полная цена') > 0);
  check('и причины больше нет', h2.indexOf('Разложения «кто был с деньгами и без них» за это окно нет') < 0);

  /* --- третье поколение: пробные посчитаны явно, остаток не нужен --- */
  const W3 = put([1, 10, 5, 0, 12, 150, 8, 1, 2, 2, 1]);
  check('и «за 0» есть', W3.split === true);
  eq('пробные взяты явно, а не остатком', W3.trial, 6);
  eq('за 0', W3.zero, 3);

  ctx._fillStore.fill = keepFill; ctx.S.fillPlan = keepFP; ctx.S.fillCats = keepCats;
  ctx._fillPeriod = keepPeriod; ctx._fillRange = keepRange; ctx.S.plan = keepPlan;
}

/* ================= ОКНО, КОТОРОЕ ЕЩЁ НЕ НАЧАЛОСЬ =================
   При from > today знаменатель схлопывался в единицу: семидневные каникулы подписывались
   «0 из 1 дня». Прошедших дней там нет вовсе, и сравнивать не с чем. */
{
  eq('окно целиком впереди', API._fillDaysNote('2026-10-26', '2026-11-01', 0), '0 дней');
  eq('и с одним посчитанным днём', API._fillDaysNote('2026-10-26', '2026-11-01', 1), '1 день');
  // а окно, которое идёт, по-прежнему считается по сегодня
  eq('окно в процессе', API._fillDaysNote('2026-09-07', '2026-09-13', 1), '1 день');
  eq('и пропуск внутри прошедших видно', API._fillDaysNote('2026-08-31', '2026-09-06', 5), '5 из 7 дней');
}
