   ...
   берётся из одной только длины отрезка и не значит ничего. */
{
  ...
  // пересечение: период внутри месяца-образца
  ctx.S.fillBase = '2026-09';
  ctx._fillPeriod = 'w:2026-08-31';
  const bad = API._fillHtml();
  check('пересечение поймано', bad.indexOf('Месяц-образец пересекается') > 0,
        bad.slice(Math.max(0, bad.indexOf('Эталон') - 20), bad.indexOf('Эталон') + 300));
  check('и объяснено, почему это бессмысленно', bad.indexOf('сравнивает период сам с собой') > 0);
