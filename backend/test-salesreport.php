...
/* ⚠️ Сколько недель легло в медиану — отдельный ответ. Без него интерфейс не отличает
   настоящую медиану 0.9 от запасного значения 0.9 «истории нет» и врёт в подписи
   «медиана за прошлые недели». Жанна 13.09.2026 спросила ровно про этот расчёт. */
$n = null; alfa_sales_ratio($reports, 8, $n);
ok('недель легло в медиану', $n, 3);
$n = null; alfa_sales_ratio([], 8, $n);
ok('без истории — ни одной', $n, 0);
$n = null;
alfa_sales_ratio(['a' => ['fact' => 0, 'prevForecast' => 100], 'b' => ['fact' => 50, 'prevForecast' => 0]], 8, $n);
ok('недели без пары не считаются', $n, 0);
/* ⚠️ Предел в восемь недель важен: медиана за полгода — уже не про сегодняшний клуб. */
$many = [];
for ($i = 1; $i <= 12; $i++) $many['2025-01-' . str_pad((string)$i, 2, '0', STR_PAD_LEFT)] = ['fact' => 900, 'prevForecast' => 1000];
$n = null; alfa_sales_ratio($many, 8, $n);
ok('дальше восьми недель не заглядываем', $n, 8);
