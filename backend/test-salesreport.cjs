...
/* ⚠️ Жанна 13.09.2026: «Доходимость 103%. Это как?» — доходимость по смыслу не бывает
   больше ста, а здесь считается выполнение плана, и перевыполнить его можно. Слово в этом
   же приложении занято дважды под другое (рейтинг менеджеров, планировщик), поэтому здесь
   его быть не должно. */
check('выполнение плана посчитано от «шли на»', API._salesWeekText(rep).indexOf('Выполнение плана 76%.') > 0,
      API._salesWeekText(rep));
check('слова «доходимость» в отчёте продаж больше нет',
      API._salesWeekText(rep).toLowerCase().indexOf('доходимост') < 0, API._salesWeekText(rep));
/* ⚠️ Ровно та строка, о которую споткнулась Жанна: 22 201 при цели 21 500. Перевыполнение —
   законный результат, и показывать его надо, а не обрезать сотней. */
rep.fact = 22201; rep.goal = 21500;
check('план перевыполнен — так и пишем', API._salesWeekText(rep).indexOf('Выполнение плана 103%.') > 0,
      API._salesWeekText(rep));
rep.fact = 16885; rep.goal = 22000;
   ...
   страшнее, чем не отправить ничего. */
{
  ...
  /* ⚠️ ДВА ОКНА — ОДНО ПРАВИЛО ЧИСЛИТЕЛЯ, И В ЧАТЕ ТОЖЕ.
     Галочки «что считать занятым местом» применимы только к окну с разложением по деньгам.
     У прошлогоднего его обычно нет — дни подтянуты до появления этих полей. Раньше строка
     сравнивала «88% по галочкам» с «63% по списаниям» и отправляла отделу продаж рост,
     которого не было. Теперь при перекосе считаем оба окна по списаниям и называем ОБА
     процента в самой строке: иначе рядом со строкой «Загрузка: 88%» стояло бы «63%, разница
     +0 п.п.» — и это читалось бы как противоречие. */
  const NEWROW = [1, 8, 5, 0, 7, 150, 5, 2, 0, 0, 2, {}, {}];   // есть attZero
  const OLDROW = [1, 8, 5, 0, 7, 150];                           // старый формат
  ctx.S.fillPlan = { '10': 8 };
  ctx.S.fillCats = { full: 1, trial: 0, zero: 1, none: 1, miss: 1 };
  const mkFill = (nowRow, lyRow) => {
    const F = {};
  ...
  // перекос: сейчас новый формат, год назад старый — данные по сути одинаковые
  ctx._fillStore.fill = mkFill(NEWROW, OLDROW);
  const skew = API._salesFillLines(repW);
  ...
  const load = okLines.find(x => x.indexOf('Загрузка:') === 0) || '';
  check('«Загрузка» и «Год назад» сходятся', load.indexOf('88%') > 0, load + ' | ' + okLy);

  /* ⚠️ Подпись окна через Новый год: год обязателен у обеих дат. Раньше он брался из даты
     конца, и «свой период» 29.12.2025–04.01.2026 уходил в чат как «29.12–04.01.2026». */
  eq('окно внутри года', API._fillRangeLabel('2025-09-01', '2025-09-07'), '01.09–07.09.2025');
  eq('окно через Новый год', API._fillRangeLabel('2025-12-29', '2026-01-04'), '29.12.2025–04.01.2026');
